...
class Onxshop_Controller_Bo_Component_Ecommerce_Product_In_Node extends Onxshop_Controller {
	...
	public function mainAction() {
		...
		if (is_array($product_homepage)) {
			...
			//breadcrumb
			$_nSite = new nSite("component/breadcrumb~id={$product_homepage['id']}:create_last_link=1~");
			$this->tpl->assign('BREADCRUMB', $_nSite->getContent());
			
			//children node list
			$_nSite = new nSite("bo/component/node_list~id={$product_homepage['id']}:node_group=content~");
